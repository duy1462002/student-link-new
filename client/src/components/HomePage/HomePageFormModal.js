...
class HomePageFormModal extends Component {
  ...
  handleSubmit = (event) => {
    ...
    const { description, locationName, coordinates, croppedImage, document } =
      this.state;
  ...
  render() {
    const { user, uploading, divs } = this.props;
    const {
      open,
      photo,
      previewPhoto,
...
export default connect(mapStateToProps)(HomePageFormModal);

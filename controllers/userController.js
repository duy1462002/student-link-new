...
const Roles = mongoose.model("OrgRole");
    ...
    if (!user) {
      bcrypt.hash(req.body.password, 10, (err, hash) => {
        if (err) {
          ...
          Roles.findOne({ code: roleConstant.REGISTERED_MEMBER }).then(
            (role) => {
              ...
              user
                ...
                .then((user) => {
                  ...
                  Promise.all([following, followers, userRoles]).then(() => {
      ...
      bcrypt.hash(req.body.password, 10, (err, hash) => {
        if (err) {
          ...
          user.save().then((user) => {
            ...
            Roles.findOne({ code: roleConstant.REGISTERED_MEMBER })
              .then((role) => {
                if (role) {
                  console.log("Role: ", role);
                  let roleId = role.id;
                  ...
                  return Promise.all([following, followers, userRoles]);
                } else {
                  console.log("Role is null, skipping UserRoles creation.");
    ...
    .then((users) => {
      if (users.length < 1) {
        return res.status(400).json({
          message: "2001-Incorrect credentials.",
        });
      } else {
        bcrypt.compare(req.body.password, users[0].password, (err, result) => {
          if (err) {
            return res.status(400).json({
              message: "2002-Incorrect credentials.",
            });
          }
          if (result) {
          ...
          return res
            .status(400)
            .json({ message: "2003-Incorrect credentials." });
        });
      }
    })
    .catch((err) => {
...
exports.getGroups = (req, res) => {
  // Aggregate query to find groups with a specific user in the members array
  ...
  // return Group.aggregate([
  //   {
  //     $match: { owner: mongoose.Types.ObjectId(req.userData.userId) }
  //   },
  ...
  //       description: 1,
  //       // memberCount: {
  //       //   $size: { $arrayElemAt: ["$groups.members.user", 0] }
  //       // }
  //     },
  //   },
  // ])
